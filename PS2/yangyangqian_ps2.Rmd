...
dcast(q3$d[,.N,by=c("C1","Block")], C1~Block)
# per the prompt, can assume clusters 9, 11, 13 (block 2) are in treatment, and clusters 1, 3, 5, 7 (block 1) are in control

# calculate the observed Y and assignment variable, given our assignment to treatment and control by clustering 1
q3$d <- q3$d[,D.C1 := ifelse(C1 %in% c(9,11,13), 1,0)][,Y.C1 := ifelse(D.C1==1, Y1,Y0)]
# q3$d[,.(C1,Y0,Y1,D.C1,Y.C1)]
