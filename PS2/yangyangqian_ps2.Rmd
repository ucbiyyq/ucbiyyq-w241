...
q5$ci <- c(q5$ci.lower, q5$ci.upper)
...
```{r}
# calculate the ci from the OLS model
confint(q5$m1)
q5$m1$ci <- confint(q5$m1)[c(2,4)] # (-20.84416, -3.567603)

# check if the OLS answers match up with the analytic answers
q5$m1$ci - q5$ci # differences are somewhat close to zero compared to the actual values, so still probably matches
